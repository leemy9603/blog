...
var select = {
	...
	selectAll: function(client,art,callback){
		console.log(art)
		var sql = 'SELECT * FROM article WHERE art_title like "%'+art.keyWord+'%"';
		client.query(sql,art,function(err,res,fields){
			if (err){
				console.log(err);
			} else{
				jsonWrite(callback,res);
	...
	selArtByUid: function(client,art,callback){
		console.log(art)
		var sql = 'SELECT * FROM article WHERE uid = "'+art.uid+'"';
		client.query(sql,art,function(err,res,fields){
			if (err){
				console.log(err);
			} else{
...
router.post('/updateArt',function(req,res){
	...
	console.log("here is the parmas")
	console.log(params);
	update.updateArt(client,params,res);
});
router.post('/selArtByUid',function(req,res){
	console.log(req)
	var params = req.body;
	console.log("here is the parmas")
	console.log(params);
	select.selArtByUid(client,params,res);
});
